...
### Example: a requested repository change

1. In ChatGPT Work, say: “Aphrael, have Work add a short usage example to the
   Aphrael repository.” Work sends that turn through `APHRAEL.ps1 ask` and keeps
   the returned Hermes `session_id` for the next Aphrael turn.
2. Aphrael delegates through `aphrael_work_delegate` and returns a bridge
   `request_id` and PR URL. Save those alongside the Hermes session ID; they
   identify different records.
3. Later, run `APHRAEL.ps1 work-status <request_id> --compact` or ask Aphrael
   in the resumed session to check that request. `pending` means there is no
   verified Work result yet.
4. After `completed + verified`, read the returned report and inspect the PR
   diff and checks before describing the code as reviewed, tested, or merged.
