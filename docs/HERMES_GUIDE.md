...
Hermes reuses supported Codex authentication from its private profile state; Aphrael does not access that auth store. If `auth status openai-codex` is not logged in, use Hermes' supported model/authentication flow to sign in, then launch with `.\START_APHRAEL.ps1 -Surface Chat`.
